...
# CSV Setup
CSV_FILE = "/home/root/demo/Vitals.csv"
...
def cleanup():
    ...
    logger.info(
        f"Processing remaining {parsed_data_queue.qsize()} items in queue.")
...
def parse_data(data):
    ...
    try:
        ...
        ppg_red = int.from_bytes(
            data[3:6], byteorder="little", signed=False) & 0x000FFFFF
        ppg_ir = int.from_bytes(
            data[6:9], byteorder="little", signed=False) & 0x000FFFFF
        heart_rate = data[9] if len(
            data) > 9 else latest_data_cache["heart_rate"]
        SpO2_val = data[14] if len(
            data) > 14 else latest_data_cache["SpO2_val"]
...
def get_data():
    ...
    if fetch_live_data:
        if not parsed_data_queue.empty():
            ...
            # Disable live fetching if queue is empty
            set_fetch_live_data(False)
...
def spodata():
    """API endpoint to enable CSV data fetching and reset everything smoothly."""
    global send_data, fetch_live_data, csv_data_generator, parsed_data_queue

    # Reset flags and states
    send_data = True
    fetch_live_data = False

    # Reset the CSV generator to start from the beginning
    csv_data_generator = read_csv_data()
    ...
    logger.info(
        "CSV data sending re-enabled and all states reset smoothly via SpO2 click.")

    # Send the first CSV row to update the UI immediately
    try:
        first_csv_data = next(csv_data_generator, None)
        return jsonify({"status": "reset", "data": first_csv_data})
    except StopIteration:
        return jsonify({"status": "reset", "data": None})
